...
- Streaming: typed SSE events — `run_started`, `token`, `tool_call`, `tool_result`, `sources`, `citation`, `interrupt`, `error`, `done`. Every stream ends with exactly one `done` or `error`.
